...
class QuestionValidationServiceTest {
    ...
    void shouldReturnQuestionWhenValidCode() {
        ...
        Question result = questionValidationService.getQuestionByCode(1);
    ...
    void shouldValidateGapCorrectlyWhenAnswerMatches() {
        ...
        boolean result = questionValidationService.validateGAP("1", "Correct Answer");
    ...
    void shouldValidateGapIncorrectlyWhenAnswerDoesNotMatch() {
        ...
        boolean result = questionValidationService.validateGAP("1", "Wrong Answer");

        assertFalse(result);
    }

    @Test
    void shouldValidateGapIncorrectlyWhenAnswerCaseDiffers() {
        GapOption correctOption = new GapOption();
        correctOption.setOptionText("Correct Answer");
        correctOption.setIsCorrect(true);
        when(gapOptionRepository.findByGapIdAndIsCorrectTrue(1)).thenReturn(List.of(correctOption));
        ...
        assertFalse(result); // Note: QuestionValidationService uses equals, not equalsIgnoreCase
    }
    ...
    void shouldCalculateMcScoreCorrectly() {
        ...
        int score = questionValidationService.validateMC(question, "1", userAnswers);
    ...
    @Test
    void shouldCalculateMcScoreWhenNoCorrectAnswers() {
        Question question = new Question();
        question.setPoints(10);
        ...
        when(mcAnswerRepository.findByQuestionIdAndIsCorrectTrue(1)).thenReturn(List.of(answer1));

        Map<String, String> userAnswers = Map.of("1", "0");

        int score = questionValidationService.validateMC(question, "1", userAnswers);

        // 0 correct out of 1, points 10, so 0
